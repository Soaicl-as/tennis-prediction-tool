...
export const syncData = api<SyncDataRequest, SyncDataResponse>(
  ...
  async ({ data_types, tour = 'both', days_back = 7, force_update = false }) => {
    try {
      ...
      for (const currentTour of tours) {
        ...
        if (data_types.includes('rankings')) {
          try {
            ...
            await cache.invalidatePattern(`rankings:tour:${currentTour}`);
          } catch (error) {
        ...
        if (data_types.includes('tournaments')) {
          try {
            ...
            await cache.invalidatePattern(`tournaments:tour:${currentTour}`);
          } catch (error) {
      ...
      // Invalidate player-related caches if players or matches were synced
      if (data_types.includes('players') || data_types.includes('matches')) {
        await cache.invalidatePattern('all_players');
        await cache.invalidatePattern('player_data');
        await cache.invalidatePattern('player_stats');
      }

      // Invalidate prediction caches if any data was synced
...
async function syncPlayers(tour: 'atp' | 'wta', forceUpdate: boolean): Promise<number> {
  try {
    ...
    for (let i = 0; i < apiPlayers.length; i += batchSize) {
      ...
      await Promise.all(batch.map(async (apiPlayer) => {
        try {
          ...
          const existingPlayer = await tennisDB.queryRow<{id: number, updated_at: string}>`
            SELECT id, updated_at FROM players WHERE LOWER(name) = LOWER(${apiPlayer.name})
          `;
          ...
          if (!existingPlayer) {
            ...
            await cache.invalidatePlayerData(existingPlayer.id);
          }
        } catch (error) {
      ...
      await Promise.all(batch.map(async (match) => {
        try {
          ...
          const [player1, player2] = await Promise.all([
            getOrCreatePlayer(match.player1),
            getOrCreatePlayer(match.player2)
          ]);
